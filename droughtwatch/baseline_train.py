...
class CBtrain():
  ...
  def file_list_from_folder(self, folder, data_path):
  ...
  def parse_tfrecords(self,filelist, batch_size, num_epochs):
  ...
  def build_estimator_from_model_original(self,args):
  ...
  def build_estimator_from_model_test(self,args):
    model = tf.keras.Sequential()
    model.add(tf.keras.layers.InputLayer(input_shape=[65,65,10], name='image'))
