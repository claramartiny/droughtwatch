...
function Combobox(props) {
  ...
  const listboxId = React.useMemo(() => getBuiId(), []); // Handles case where an application wants to update the value in the input element
  ...
  , _extends({
    ref: rootRef
  }, rootProps), React.createElement(OverriddenPopover, _extends({
    isOpen: isOpen,
    overrides: popoverOverrides,
    placement: PLACEMENT.bottomLeft,
    content: React.createElement(ListBox // TabIndex attribute exists to exclude option clicks from triggering onBlur event actions.
    , _extends({
      tabIndex: "-1",
      ...
      return (// List items are not focusable, therefore will never trigger a key event from it.
        ...
        React.createElement(ListItem, _extends({
          ...
          $size: size
        }, listItemProps), ReplacementNode ? React.createElement(ReplacementNode, {
          isSelected: isSelected,
          option: option
        }) : mapOptionToString(option))
      );
    }))
  }, restPopoverProps), React.createElement(InputContainer, _extends({
    ...
    ,
    role: "combobox"
  }, inputContainerProps), React.createElement(OverriddenInput, _extends({
    inputRef: inputRef,
